...
class C_DBaseData():
    ...
    def getData_EqThevenin(self, versaodatabase, nomeCircuitoAlta):  # OBS: Método obsoleto
        """
        Seleciona colunas do banco CTAT de um determinado circuito e as armazena em uma lista


        :param versaodatabase: str, versão da BDGD usada pelo usuário
    ...
    def getData_CTMT_EQTH(self, versaodatabase, pac_2 = None):  # OBS: Por que o if pac_2?
        """
        ...
        try:
            match versaodatabase:
                ...
                case "2021":
                    if pac_2 is not None:
                        sqlQuery = "SELECT ten_nom, uni_tr_s, nome, cod_id  FROM ctmt  WHERE pac_ini ='" + pac_2
    ...
    def getData_CTMT(self, versaodatabase, nomeSE_MT = None):
        """
    ...
    def getData_TrafosAT_MT(self, versaodatabase, nomeSE_MT):
        """
    ...
    def getData_TRAFO_UNTRS(self, versaodatabase, uni_tr_at):
        """
    ...
    def getData_Condutores(self, versaodatabase, tipoCondutor):
        """
    ...
    def getData_SecAT(self, versaodatabase, nomeSE_MT):
        """
    ...
    def getData_SecMT(self, versaodatabase, nomeSE_MT, tipoSEC):
        """
    ...
    def getData_SegLinhasMT(self, versaodatabase, nomeSE_MT):
        """
    ...
    def getData_ReguladorMT(self, versaodatabase, nomeSE_MT):
        """
    ...
    def getData_UniConsumidora(self, versaodatabase, nomeSE_MT, tipoUniCons):
        """
    ...
    def getData_TrafoDIST(self, versaodatabase, codCT_MT): # OBS: Verificar a existência de erros nas NamedTuples aqui
        """
        ...
        try:
            match versaodatabase:
                ...
                case "2021":
                    sqlQueryTRMT = ("SELECT untrmt_cod_id, untrmt_pac_1, untrmt_pac_2, untrmt_pac_3, " +
                                    "untrmt_fas_con_p, untrmt_fas_con_s, untrmt_fas_con_t, untrmt_sit_ativ," +
                                    " untrmt_tip_unid, untrmt_ten_lin_se, untrmt_cap_elo, untrmt_cap_cha, untrmt_tap," +
                                    " untrmt_pot_nom, untrmt_per_fer, untrmt_per_tot, untrmt_ctmt," +
                                    " untrmt_tip_trafo, untrmt_uni_tr_s,"+
                                    "eqtrmt_cod_id, eqtrmt_fas_con, eqtrmt_pot_nom, eqtrmt_lig, eqtrmt_ten_pri, " +
                                    "eqtrmt_ten_sec, eqtrmt_ten_ter, eqtrmt_lig_fas_p, eqtrmt_lig_fas_s, " +
                                    "eqtrmt_lig_fas_t, eqtrmt_per_fer, eqtrmt_per_tot, eqtrmt_r, eqtrmt_xhl " +
                                    "FROM trmt WHERE ctmt = '" + codCT_MT)

                    TRMTs_ctmt = self.DataBaseConn.getSQLDB("TRMT", sqlQueryTRMT)

                    lista_TRMTs_ctmt = []
                    for TRMT_ctmt in TRMTs_ctmt:
                        TRMT_ctmt = data.dadosTrafoDist(
                            TRMT_ctmt[0],   # cod_id
                            TRMT_ctmt[1],   # pac_1
                            TRMT_ctmt[2],   # pac_2
                            TRMT_ctmt[3],   # pac_3
                            TRMT_ctmt[4],   # fas_con_s
                            TRMT_ctmt[5],   # sit_ativ
                            TRMT_ctmt[6],   # ten_lin_se
                            TRMT_ctmt[7],   # pot_nom
                            TRMT_ctmt[8],   # ctmt
                            TRMT_ctmt[9],   # uni_tr_s
                            TRMT_ctmt[10],  # uni_tr_s
                            TRMT_ctmt[11],  # uni_tr_s
                            TRMT_ctmt[12],  # uni_tr_s
                            TRMT_ctmt[13],  # uni_tr_s
                            TRMT_ctmt[14],  # uni_tr_s
                            TRMT_ctmt[15],  # uni_tr_s
                            TRMT_ctmt[16],  # uni_tr_s
                            TRMT_ctmt[17],  # uni_tr_s
                            TRMT_ctmt[18],  # uni_tr_s
                            TRMT_ctmt[19],  # pot_nom_eqtrd
                            TRMT_ctmt[1],   # lig
                            TRMT_ctmt[2],   # ten_pri
                            TRMT_ctmt[3],   # ten_pri
                            TRMT_ctmt[20],  # ten_pri
                            TRMT_ctmt[21],  # ten_pri
                            TRMT_ctmt[22],  # ten_pri
                            TRMT_ctmt[23],  # ten_pri
                            TRMT_ctmt[24],  # ten_pri
                            TRMT_ctmt[25],  # ten_pri
                            TRMT_ctmt[26],  # ten_pri
                            TRMT_ctmt[27],  # ten_pri
                            TRMT_ctmt[28],  # ten_pri
                            TRMT_ctmt[29],  # ten_pri
                            TRMT_ctmt[30],  # r
                            TRMT_ctmt[31],  # xhl
                            TRMT_ctmt[32]   # ten_pri
                        )
                        lista_TRMTs_ctmt.append(TRMT_ctmt)
    ...
    def getData_SegLinhasRamLigBT(self, versaodatabase, nomeSE_MT, tipoLinha):
        """
        Seleciona colunas dos bancos de transformadores MT de um determinado alimentador, e as armazena em uma lista
    ...
    def getData_UniCompReativo(self, versaodatabase, nomeSE_MT, tipoCAP):  # Pega os reguladores
        """
